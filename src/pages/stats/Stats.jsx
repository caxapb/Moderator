...
export default function Stats() {
  ...
  useEffect(() => {
    const fetchStatsData = async () => {
      try {
        const summaryData = await fetchData(`${API_URL}/api/v1/stats/summary?period=${period}`);
        setStats(summaryData);
        const decisionsData = await fetchData(`${API_URL}/api/v1/stats/chart/decisions?period=${period}`);
        setDecisions(decisionsData);
        const categoriesData = await fetchData(`${API_URL}/api/v1/stats/chart/categories?period=${period}`);
